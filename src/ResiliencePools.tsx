      ...
      <div className="card-body">
        <div className="flex items-center justify-between mb-4">
          <h3 className="card-title text-emerald-200 text-xl">
            <Flame className="w-6 h-6 mr-2" /> Jup.ag Studio Token • Bonding Curve Launch
          </h3>
...
// ---------- Game Integration Badges ----------
function GameIntegrationBadges() {
  const [activeGames, setActiveGames] = useState([
    { name: 'WaterDrop Catcher 88', volume: '$12,450', players: 342, boost: '+15%' },
    { name: 'AR Reef Hunt', volume: '$8,320', players: 156, boost: '+8%' },
    { name: 'Tribal Blessing Simulator', volume: '$5,670', players: 89, boost: '+12%' }
  ]);

  return (
    <div className="card bg-[#0a021d]/60 border border-fuchsia-400/30 mb-8">
      <div className="card-body">
        <div className="flex items-center justify-between mb-4">
          <h3 className="card-title text-fuchsia-200 text-xl">
            <Play className="w-6 h-6 mr-2" /> Play-to-Stake Integration
          </h3>
          <button className="btn btn-ghost btn-sm text-cyan-200">
        ...
        <div className="text-center mb-6">
          <p className="text-white/80">
            🎮 <strong>Play Games → Earn $💧 → Stake for Impact → Multiply Relief Funding</strong> 🎮
          </p>
        </div>
        
        <div className="grid md:grid-cols-3 gap-4">
          {activeGames.map((game, index) => (
            <div key={index} className="p-4 bg-[#0b0220]/70 rounded-lg border border-fuchsia-400/20 hover:border-fuchsia-400/40 transition-all cursor-pointer">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-fuchsia-200 font-semibold text-sm">{game.name}</h4>
                <span className="text-emerald-300 text-xs font-bold">{game.boost}</span>
              </div>
              <div className="space-y-1 text-xs">
                <div className="flex justify-between">
                  <span className="text-white/60">Volume:</span>
                  <span className="text-cyan-300">{game.volume}</span>
                </div>
                <div className="flex justify-between">
              ...
              <button className="btn btn-ghost btn-xs w-full mt-3 text-fuchsia-200 hover:bg-fuchsia-500/20">
        ...
        <div className="mt-6 p-4 bg-gradient-to-r from-fuchsia-500/10 via-emerald-500/10 to-cyan-500/10 rounded-lg border border-fuchsia-400/20">
          <div className="text-center">
            <div className="text-white/90 font-semibold mb-2">
              🏆 Volume Champions get bonus staking yields!
            </div>
            <div className="text-fuchsia-200 text-sm">
              Top players earn up to 25% additional APY on their staked positions
            </div>
          </div>
        </div>
      </div>
...
function ActiveStakingPositions() {
  const [positions, setPositions] = useState([
    {
      id: 1,
      organization: "Jamaica Disaster Relief Foundation",
      amount: 5000,
      term: 14,
      daysRemaining: 8,
      pendingYield: 287.50,
    ...
    {
      id: 2,
      organization: "Caribbean Climate Resilience Initiative", 
      amount: 2500,
      term: 10,
      daysRemaining: 3,
      pendingYield: 103.75,
  ...
  return (
    <div className="card bg-[#0b0220]/70 border border-emerald-400/30 mb-8">
      <div className="card-body">
        <div className="flex items-center justify-between mb-6">
          <h3 className="card-title text-emerald-200 text-xl">
            <Target className="w-6 h-6 mr-2" /> My Staking Positions
          </h3>
          <div className="text-emerald-300 font-bold">
            Total: ${positions.reduce((sum, p) => sum + p.amount, 0).toLocaleString()}
          </div>
        </div>
        
        <div className="space-y-4">
          {positions.map((position) => (
            <div key={position.id} className="p-4 bg-[#0a021d]/50 rounded-lg border border-emerald-400/20">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h4 className="text-emerald-200 font-semibold">{position.organization}</h4>
                  <div className="text-white/60 text-sm">
                    ${position.amount.toLocaleString()} • {position.term} days • {position.daysRemaining} days remaining
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-emerald-300 font-bold">
                    +${position.pendingYield.toFixed(2)}
                  </div>
                  <div className="text-white/60 text-xs">Pending Yield</div>
                </div>
              </div>
              
              <div className="flex gap-2">
                <button className="btn btn-primary btn-sm bg-gradient-to-r from-emerald-500 to-cyan-500 border-0">
                  <ArrowRight className="w-3 h-3 mr-1" /> Stake More
                </button>
                {position.donationEligible && (
                  <button className="btn btn-secondary btn-sm bg-gradient-to-r from-fuchsia-500 to-emerald-500 border-0">
                    <Heart className="w-3 h-3 mr-1" /> Donate Now
                  </button>
                )}
                <button className="btn btn-ghost btn-sm text-white/70">
                  Details
                </button>
              </div>
            </div>
...
export default function ResiliencePools() {
  ...
  const navigateToGuide = () => {
    window.location.hash = '/initiative-guide';
  };
    ...
    <div className="min-h-screen text-white selection:bg-emerald-400/30 selection:text-white">
      ...
      <div className="max-w-6xl mx-auto px-4 pb-8">
        <div className="tabs tabs-boxed bg-[#09021a]/60 border border-emerald-400/20 max-w-2xl mx-auto">
          ...
          <button 
            className={`tab flex-1 ${activeTab === 'games' ? 'tab-active bg-gradient-to-r from-emerald-500/30 to-cyan-500/30 text-white' : 'text-white/70'}`} 
            onClick={() => setActiveTab('games')}
          >
            <Play className="w-4 h-4 mr-2" /> Play-to-Stake
          </button>
          <button 
      ...
      <div className="max-w-6xl mx-auto px-4 pb-16">
        ...
        {activeTab === 'games' && (
          <div className="space-y-8">
            {/* Game Integration Badges */}
            <GameIntegrationBadges />
            
            {/* Impact Multiplication */}
            <ImpactMultiplicationViz />
            
            {/* Leaderboard */}
            <Section title="Volume Champions Leaderboard" subtitle="Top contributors earn bonus staking yields">
              <div className="card bg-[#0b0220]/70 border border-fuchsia-400/30">
                <div className="card-body">
                  <div className="space-y-4">
                    {[
                      { rank: 1, name: "Reef Guardian DAO", volume: "$45,230", boost: "+25%", impact: 98 },
                      { rank: 2, name: "Jamaica Diaspora", volume: "$38,450", boost: "+20%", impact: 94 },
                      { rank: 3, name: "Climate Champions", volume: "$32,120", boost: "+15%", impact: 87 },
                      { rank: 4, name: "Ocean Stewards", volume: "$28,670", boost: "+10%", impact: 82 },
                      { rank: 5, name: "Water Warriors", volume: "$24,330", boost: "+5%", impact: 76 }
                    ].map((champion) => (
                      <div key={champion.rank} className="flex items-center justify-between p-4 bg-[#0a021d]/50 rounded-lg border border-fuchsia-400/20">
                        <div className="flex items-center gap-4">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${
                            champion.rank === 1 ? 'bg-yellow-500 text-black' :
                            champion.rank === 2 ? 'bg-gray-400 text-black' :
                            champion.rank === 3 ? 'bg-orange-500 text-black' :
                          ...
                          <div>
                            ...
                            <div className="text-fuchsia-300 text-sm">{champion.volume} volume this month</div>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-emerald-200 font-bold">{champion.boost}</div>
                          <div className="text-white/60 text-xs">Staking Boost</div>
                        </div>
                      </div>
                    ))}
                  </div>
